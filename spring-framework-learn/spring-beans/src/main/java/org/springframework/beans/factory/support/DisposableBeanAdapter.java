 ...
 */
// 实现在特定bean实例上执行各种销毁步骤的DisposableBean和Runnable接口的适配器
...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	// 缓存DestructionAwareBeanPostProcessor实例
	@Nullable
	...
	public DisposableBeanAdapter(Object bean, String beanName, RootBeanDefinition beanDefinition,
		...
		// 是DisposableBean实例，并且没有外部销毁方法，或外部方法中不包含destroy方法
	 ...
	 */
	// 如果给定beanDefinition的“ destroyMethodName”属性的当前值为AbstractBeanDefinition.INFER_METHOD ，
	// 则尝试推断一个destroy方法。 候选方法目前仅限于名为“ close”或“ shutdown”（无论是在本地声明还是继承）的公共无参数方法。
	// 如果找不到这样的方法，则将给定的BeanDefinition的“ destroyMethodName”更新为null，否则将其设置为推断的方法的名称。
	// 该常量用作@Bean#destroyMethod属性的默认值，并且该常量的值也可以在XML中的<bean destroy-method="">或<beans default-destroy-method="">属性中使用。
	//还处理java.io.Closeable和AutoCloseable接口，并在实现bean时反射地调用“ close”方法
	@Nullable
	...
	public static boolean hasDestroyMethod(Object bean, RootBeanDefinition beanDefinition) {
		if (bean instanceof DisposableBean || bean instanceof AutoCloseable) {
			// 判断这个bean 是否是DisposableBean 或 AutoCloseable接口实现类，如果是，则判断有销毁方法
			return true;
		}
		String destroyMethodName = beanDefinition.getDestroyMethodName();
		if (AbstractBeanDefinition.INFER_METHOD.equals(destroyMethodName)) {
			// 销毁方法为(inferred) 推断，如果类中有close shutdown方法，也认为有销毁方法
			return (ClassUtils.hasMethod(bean.getClass(), CLOSE_METHOD_NAME) ||
					ClassUtils.hasMethod(bean.getClass(), SHUTDOWN_METHOD_NAME));
		}

		// 如果方法名不是inferred，并且有方法名字，则也认为有销毁方法
