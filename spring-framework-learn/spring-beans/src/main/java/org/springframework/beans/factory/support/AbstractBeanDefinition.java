...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 */
	// 指示容器应尝试推断bean的destroy method name常量，与显式指定方法名称相反。
	// 值“（推断）”专门设计为在方法名称中包含否则为非法的字符，从而确保与具有相同名称的合法命名方法不发生冲突的可能性。
	...
	// 标记bean definition是否可访问非公共的方法（包括构造方法）
	 ...
	 */
	// 返回构造函数是在什么模式下解析的。2种模式：宽松模式和严格模式；默认是宽松模式
