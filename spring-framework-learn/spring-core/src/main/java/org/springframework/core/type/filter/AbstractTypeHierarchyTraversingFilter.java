 ...
 */
// 遍历层次结构的过滤器。
//当需要基于潜在的整个类/接口层次结构进行匹配时，此过滤器很有用。 所采用的算法采用了快速成功策略：如果在任何时候声明匹配，将不执行进一步的处理
public abstract class AbstractTypeHierarchyTraversingFilter implements TypeFilter {
	...
	public boolean match(MetadataReader metadataReader, MetadataReaderFactory metadataReaderFactory)
		...
		// This method optimizes avoiding unnecessary creation of ClassReaders 此方法优化了避免不必要的ClassReaders创建的方法
		// as well as visiting over those readers. 以及访问那些reader
